...
resource "google_monitoring_alert_policy" "istio_service_availability_slo_alert" {
  ...
  conditions {
    ...
    condition_threshold {
      ...
      filter          = "select_slo_burn_rate(\"projects/${var.project_id}/services/ist:${var.project_id}-location-${var.zone}-cloud-ops-sandbox-default-${var.istio_services[count.index].service_id}/serviceLevelObjectives/${google_monitoring_slo.istio_service_availability_slo[count.index].slo_id}\", 60m)"
...
resource "google_monitoring_alert_policy" "istio_service_latency_slo_alert" {
  ...
  conditions {
    ...
    condition_threshold {
      filter          = "select_slo_burn_rate(\"projects/${var.project_id}/services/ist:${var.project_id}-location-${var.zone}-cloud-ops-sandbox-default-${var.istio_services[count.index].service_id}/serviceLevelObjectives/${google_monitoring_slo.istio_service_latency_slo[count.index].slo_id}\", 60m)"
      threshold_value = var.istio_services[count.index].availability_burn_rate
      comparison      = "COMPARISON_GT"
      duration        = "60s"
    }
